...
t_int *blotar_perform(t_int *w)
{
	...
	while(n--) {
		...
		pressureDiff = JetTabl_lookup(pressureDiff + (er * temp)); //becomes "tube" distortion
...
static void pluck(t_blotar *x, float amplitude, float position)
{ /* this function gets interesting here, */
  /* because pluck may be longer than     */
  /* string length, so we just reset the  */
  /* soundfile and add in the pluck in    */
...
static void blotar_bang(t_blotar *x)
{
	x->pluck = 1;
}
...
static void blotar_float(t_blotar *x, t_floatarg f)
{
		x->pluckAmp = f;
}

static void blotar_pluckPos(t_blotar *x, t_floatarg f)
{
	x->pluckPos = f;
}

		
static void blotar_bodySize(t_blotar *x, t_floatarg f)
{
	x->bodySize = f;
}

		
static void blotar_bp(t_blotar *x, t_floatarg f)
{
	x->x_bp = f;
}

		
static void blotar_jd(t_blotar *x, t_floatarg f)
{
	x->x_jd = f;
}

		
static void blotar_ng(t_blotar *x, t_floatarg f)
{
	x->x_ng = f;
}

		
static void blotar_vf(t_blotar *x, t_floatarg f)
{
	x->x_vf = f;
}

		
static void blotar_va(t_blotar *x, t_floatarg f)
{
	x->x_va = f;
}

		
static void blotar_freq(t_blotar *x, t_floatarg f)
{
	x->x_fr = f;
}

static void blotar_er(t_blotar *x, t_floatarg f)
{
	x->x_er = f;
}

		
static void blotar_jr(t_blotar *x, t_floatarg f)
{
	x->x_jr = f;
}
...
{
    blotar_class = class_new(gensym("blotar~"), (t_newmethod)blotar_new, (t_method)blotar_free,
        sizeof(t_blotar), 0, 0);
    ...
    class_addmethod(blotar_class, (t_method)blotar_pluckPos, gensym("pluckPos"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_bodySize, gensym("bodySize"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_bp, gensym("bp"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_jd, gensym("jd"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_ng, gensym("ng"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_vf, gensym("vf"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_va, gensym("va"), A_FLOAT, A_NULL);
	class_addmethod(blotar_class, (t_method)blotar_freq, gensym("freq"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_jr, gensym("jr"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_er, gensym("er"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)blotar_filterRatio, gensym("filterRatio"), A_FLOAT, A_NULL);
    class_addmethod(blotar_class, (t_method)setmic, gensym("setmic"), A_FLOAT, A_NULL);
}
#endif /* PD */
