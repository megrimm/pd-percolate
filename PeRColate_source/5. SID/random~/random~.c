...
static void random_range(t_random *x, t_floatarg n)
{
		x->range = n;
	    if (x->range<1) x->range = 1;
	    if (x->range>65535) x->range = 65535;
...
// instantiate the object
static void *random_new(long flag)
{	
...
void random_tilde_setup(void)
{
    random_class = class_new(gensym("random~"), 
        (t_newmethod) random_new, 0,
        sizeof(t_random), 0, A_DEFFLOAT, A_NULL);

    class_addmethod(random_class, nullfn, gensym("signal"), 0);
    class_addmethod(random_class, (t_method)random_dsp, gensym("dsp"), 0);
    class_addmethod(random_class, (t_method)random_range, gensym("range"), A_FLOAT, 0);
	class_addfloat(random_class, (t_method)random_range);
    class_sethelpsymbol(random_class, gensym("help-random~.pd"));
}
#endif /* PD */
