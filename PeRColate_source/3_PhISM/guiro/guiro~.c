...
static void guiro_float(t_guiro *x, t_floatarg f)
{
	x->num_objects = f;

}

static void guiro_res_freq(t_guiro *x, t_floatarg f)
{
	x->res_freq = f;
}

static void guiro_shake_damp(t_guiro *x, t_floatarg f)
{
	x->shake_damp = f;
}

static void guiro_shake_max(t_guiro *x, t_floatarg f)
{
	x->shake_max = f;
}

static void guiro_res_freq2(t_guiro *x, t_floatarg f)
{
	x->res_freq2 = f;
}
...
static void guiro_bang(t_guiro *x)
{
	int i;
	post("guiro: scraping");
	x->guiroScrape = 0.;
...
void guiro_tilde_setup(void)
{
    ...
    class_addmethod(guiro_class, (t_method)guiro_res_freq, gensym("res_freq"), A_FLOAT, A_NULL);
    class_addmethod(guiro_class, (t_method)guiro_shake_damp, gensym("shake_damp"), A_FLOAT, A_NULL);
    class_addmethod(guiro_class, (t_method)guiro_shake_max, gensym("shake_max"), A_FLOAT, A_NULL);
    class_addmethod(guiro_class, (t_method)guiro_res_freq2, gensym("res_freq2"), A_FLOAT, A_NULL);
}
#endif /* PD */
