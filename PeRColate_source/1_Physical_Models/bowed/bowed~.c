...
static void setFreq(t_bowed *x, float freq)
{
	if(freq < 20.) freq = 20.;
	x->baseDelay = x->srate / freq - 4.0;   	/* delay - approx. filter delay */
	DLineL_setDelay(&x->bridgeDelay, x->baseDelay * x->betaRatio); /* bow to bridge length */
...
//vib funcs
static void setVibFreq(t_bowed *x, float freq)
{
	x->vibRate = VIBLENGTH * x->one_over_srate * freq;
}

static float vib_tick(t_bowed *x)
{
	long temp;
	float temp_time, alpha, output;
...
static t_int *bowed_perform(t_int *w)
{
	t_bowed *x = (t_bowed *)(w[1]);
...
static void bowed_dsp(t_bowed *x, t_signal **sp)
{
	x->srate = sp[0]->s_sr;
    x->one_over_srate = 1./x->srate;
    OnePole_setPole(&x->reflFilt, 0.6 - (0.1 * 22050. / x->srate));
	dsp_add(bowed_perform, 3, x, sp[1]->s_vec, sp[0]->s_n);	
	
}



static void bowed_float(t_bowed *x, t_floatarg f)
{
		x->x_bp = f;
}

static void bowed_bpos(t_bowed *x, t_floatarg f)
{
	x->x_bpos = f;
}

		
static void bowed_bv(t_bowed *x, t_floatarg f)
{
	x->x_bv = f;
}

		
static void bowed_vf(t_bowed *x, t_floatarg f)
{
	x->x_vf = f;
}

		
static void bowed_va(t_bowed *x, t_floatarg f)
{
	x->x_va = f;
}

		
static void bowed_freq(t_bowed *x, t_floatarg f)
{
	x->x_fr = f;
}

static void bowed_free(t_bowed *x)
{
	DLineL_free(&x->bridgeDelay);
	DLineL_free(&x->neckDelay);
}

static void *bowed_new(void)
{
	unsigned int i;

    t_bowed *x = (t_bowed *)pd_new(bowed_class);
    
    //zero out the struct, to be careful (takk to jkclayton)
    if (x) { 
        for(i=sizeof(t_object);i<sizeof(t_bowed);i++)  
                ((char *)x)[i]=0; 
	} 
	//x->x_obj.z_misc  = Z_NO_INPLACE;
...
{
	bowed_class = class_new(gensym("bowed~"), (t_newmethod)bowed_new, (t_method)bowed_free,
        sizeof(t_bowed), 0, 0);
    class_addmethod(bowed_class, nullfn, gensym("signal"), A_NULL);
    class_addmethod(bowed_class, (t_method)bowed_dsp, gensym("dsp"), A_NULL);
	class_addfloat(bowed_class, (t_method)bowed_float);
    class_addmethod(bowed_class, (t_method)bowed_bpos, gensym("bpos"), A_FLOAT, A_NULL);
    class_addmethod(bowed_class, (t_method)bowed_bv, gensym("bv"), A_FLOAT, A_NULL);
    class_addmethod(bowed_class, (t_method)bowed_vf, gensym("vf"), A_FLOAT, A_NULL);
    class_addmethod(bowed_class, (t_method)bowed_va, gensym("va"), A_FLOAT, A_NULL);
    class_addmethod(bowed_class, (t_method)bowed_freq, gensym("freq"), A_FLOAT, A_NULL);
}	
#endif /* PD */
