...
static void mando_float(t_mando *x, t_floatarg f)
{
		x->pluckAmp = f;
}

static void mando_pluckPos(t_mando *x, t_floatarg f)
{
	x->pluckPos = f;
}
...
static void mando_detuning(t_mando *x, t_floatarg f)
{
	x->detuning = f;
}

		
static void mando_bodySize(t_mando *x, t_floatarg f)
{
	x->bodySize = f;
}

		
static void mando_freq(t_mando *x, t_floatarg f)
{
	x->x_fr = f;
}
...
{
    mando_class = class_new(gensym("mandolin~"), (t_newmethod)mando_new, (t_method)mando_free,
        sizeof(t_mando), 0, 0);
    ...
    class_addmethod(mando_class, (t_method)mando_pluckPos, gensym("pluckPos"), A_FLOAT, A_NULL);
    class_addmethod(mando_class, (t_method)mando_stringDamping, gensym("stringDamping"), A_FLOAT, A_NULL);
    class_addmethod(mando_class, (t_method)mando_detuning, gensym("detuning"), A_FLOAT, A_NULL);
    class_addmethod(mando_class, (t_method)mando_bodySize, gensym("bodySize"), A_FLOAT, A_NULL);
    class_addmethod(mando_class, (t_method)mando_freq, gensym("freq"), A_FLOAT, A_NULL);
	class_addmethod(mando_class, (t_method)setmic, gensym("setmic"), A_FLOAT, 0);
}
