...
{
  tamb_class = class_new(gensym("tamb~"), (t_newmethod)tamb_new, 0, (short)sizeof(t_tamb), 0L, A_DEFFLOAT, 0);
  class_addmethod(tamb_class, (t_method)tamb_dsp, gensym("dsp"), 0);
  class_addfloat(tamb_class, (t_method)tamb_float);
  class_addmethod(tamb_class, (t_method)tamb_res_freq, gensym("res_freq"), A_FLOAT, 0);
  class_addmethod(tamb_class, (t_method)tamb_res_freq1, gensym("res_freq1"), A_FLOAT, 0);
  class_addmethod(tamb_class, (t_method)tamb_res_freq2, gensym("res_freq2"), A_FLOAT, 0);
  class_addmethod(tamb_class, (t_method)tamb_shake_dump, gensym("shake_dump"), A_FLOAT, 0);
  class_addmethod(tamb_class, (t_method)tamb_shake_max, gensym("shake_max"), A_FLOAT, 0);
...
static void *tamb_new(double initial_coeff)
{
    ...
    if (x) { 
#ifdef MSP
      for(i=sizeof(t_pxobject);i<sizeof(t_tamb);i++) ((char *)x)[i]=0; 
#endif
#ifdef PD
      for(i=sizeof(t_object);i<sizeof(t_tamb);i++) ((char *)x)[i]=0; 
#endif
    } 
#ifdef MSP
